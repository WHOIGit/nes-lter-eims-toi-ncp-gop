...
for (i in 1:nrow(cruise_list)){
  ...
  ## TODO: add Armstrong depth
  # function identifies toi source based on Niskin value and assigns underway sample depth to 5 m (from 0 m)
...
# combine all cruises
bottle <- rbind(toi_EN608, toi_EN617, toi_AR31A, toi_EN627, toi_EN644)
...
if (any(bottle$depth_matlab == 0)) {
  toi_niskin_0 <- bottle[which(bottle$depth_matlab == 0),]
  print(toi_niskin_0)
}
toi_niskin_0$reason <- "Niskin with provided depth = 0"

# check for any underway samples with depth not 5 m 
toi_underway_depth <- bottle %>% filter(toi_source == "toi_underway" & depth_matlab != 5.0)
...
missingdates <- rbind(missingdate_EN608, missingdate_EN617, missingdate_AR31A, missingdate_EN627, missingdate_EN644)
# these are are row 1114 in input file RaEn608withbiosat and row 9845 in input file RaEn627withbiosat 

# TODO: separate by year
...
eims_all <- rbind(eims_EN608, eims_EN617, eims_AR31A, eims_EN627, eims_EN644)
...
lat_col <- eims_edi$latitude_matlab
lon_col <- eims_edi$longitude_matlab
