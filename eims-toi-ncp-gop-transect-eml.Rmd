...
# combine all cruises
bottle <- rbind(toi_EN608, toi_EN617, toi_AR31A, toi_EN627,  toi_AR34B, toi_EN644)
...
# isolate and remove rows with time difference greater than 60 minutes
excl_time_conflicts <- bottle %>% filter(time_diff_min > 60)
excl_time_conflicts$reason <- "large time difference"
bottle <- bottle %>% filter(time_diff_min < 60 | is.na(time_diff_min))

# isolate instances of large depth differences
bottle$depth_diff <- bottle$depth_matlab - bottle$depth_API
# remove rows with large depth differences
excl_depth_conflicts <- bottle %>% filter(depth_diff > 5 | depth_diff < -5)
excl_depth_conflicts$reason <- "large depth difference"
bottle <- bottle %>% filter(abs(depth_diff) < 5 | is.na(depth_diff))

# isolate and remove the niskins not found in API. This should be the same as samples printed in loop above except if removed in rbind chunk
excl_api_no_niskin <- bottle %>% filter(toi_source == "toi_niskin" & is.na(depth_API))
excl_api_no_niskin$reason <- "Niskin not found in API"
bottle <- bottle %>% filter((toi_source == "toi_niskin" & !is.na(depth_API)) | toi_source == "toi_underway")

# isolate and remove samples recorded as Niskin with depth of 0
excl_niskin_0 <- bottle %>% filter(toi_source == "toi_niskin" & depth_matlab == 0)
excl_niskin_0$reason <- "Niskin with provided depth = 0"
bottle <- bottle %>% filter((toi_source == "toi_niskin" & depth_matlab != 0) | toi_source == "toi_underway")

# isolate and remove any underway samples with provided depth not 0 m 
...
bottle <- bottle %>% filter((toi_source == "toi_underway" & depth_matlab == 0) | toi_source == "toi_niskin")
...
bottle <- bottle %>% filter(!is.na(datetime_utc))
...
bottle_excluded <- rbind(excl_niskin99,
                         excl_bottlecasttime,
                         excl_time_conflicts,
                         ...
                         excl_underway_depth,
                         excl_missing_time)
write.csv(bottle_excluded, "toi_bottle_excluded.csv")
...
# Plot measured parameters in bottle TOI dataset
ggplot(bottle, aes(x= cruise_day, y = O2_Ar_ratio, color = cruise))+ geom_line()
ggplot(bottle, aes(x= cruise_day, y = O2_Ar_delta, color = cruise))+ geom_line()
ggplot(bottle, aes(x= cruise_day, y = cap_Delta_17O, color = cruise))+ geom_line()
ggplot(bottle, aes(x= cruise_day, y = d17O, color = cruise))+ geom_line()
ggplot(bottle, aes(x= cruise_day, y = d18O, color = cruise))+ geom_line()
...
this_pkg <- cruise_list %>% filter(package_id == "knb-lter-nes.14.2")
...
```
# Exclude ncp-gop samples affected by incorrect cast start time
...
ncp_gop_input <- ncp_gop_input %>%
   mutate(cast_flag = case_when(datetime_utc_matlab == "2018-02-02 09:10:00" ~ 1, #EN608 large depth difference
                                datetime_utc_matlab == "2018-02-01 17:31:00" ~ 1, # EN608
                                datetime_utc_matlab == "2018-07-22 05:00:00" ~ 1, #EN617
                                datetime_utc_matlab == "2018-07-22 08:58:00" ~ 1, #EN617
                                datetime_utc_matlab == "2018-07-23 12:59:00"  ~ 1, #EN617
                                datetime_utc_matlab == "2018-07-24 15:33:00"  ~ 1, #EN617
                                datetime_utc_matlab == "2018-10-21 17:56:00" & niskin == 2 ~ 1, # AR31A large depth difference
                                datetime_utc_matlab == "2018-10-21 17:56:00" & niskin == 10 ~ 1, # AR31A large depth difference
                                is.na(datetime_utc_matlab) ~ 1, #AR31A missing time
                                datetime_utc_matlab == "2019-08-22 00:57:00"  ~ 1, #EN644
                                ...
                                datetime_utc_matlab == "2019-08-21 00:48:00" ~ 1, # EN644 underway sample with non-zero provided depth
                                datetime_utc_matlab == "2019-02-01 21:39:00" & niskin == 2 ~ 1, # EN627 large depth difference
                                datetime_utc_matlab == "2019-02-02 06:29:00" & niskin == 1 ~ 1, # EN627 large depth difference
                                datetime_utc_matlab == "2019-02-03 17:42:00" & depth == 0 ~ 1, # EN627 Niskin with provided depth zero
                                datetime_utc_matlab == "2019-04-22 04:34:00"  ~ 1, #AR34B
                                TRUE ~ 0,))
...
toi_transect_surface <- filter(toi_transect, depth_matlab <= 6)
