...
#write.csv(toi_depth_conflicts, "check_toi_depth_conflicts.csv")

# isolate the niskins not found in API. This should be the same as samples printed in loop above
toi_api_no_niskin <- bottle %>% filter(toi_source == "toi_niskin" & is.na(depth_API))
#write.csv(toi_api_no_niskin, "check_niskins_not_api.csv")
...
toi_niskin_check <- rbind(toi_depth_conflicts, toi_api_no_niskin, toi_niskin_0)
write.csv(toi_niskin_check, "toi_niskin_check.csv")
...
# combine loop output from all cruises into one dataframe
...
# convert Niskin of 99 and depth of 999 to NA
bottle <- bottle %>%
  mutate(depth_matlab = replace(depth_matlab, depth_matlab=="999", NA)) %>%
  mutate(niskin = replace(niskin, niskin=="99", NA)) 
...
this_pkg <- cruise_list %>% filter(package_id == "knb-lter-nes.14.1")
