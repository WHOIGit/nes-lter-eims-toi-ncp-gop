...
```
# Add datetime from API for bottle samples 

# this can't work this way right now because we don't have niskin, depth, or toi source in input file 
```{r}
...
# 
# # create columns to populate from API
# ncp_gop$cast <- NA_integer_
# ncp_gop$depth_API <- NA_integer_
# ncp_gop$datetime_utc_API <- as.POSIXct(NA)
# 
# for (i in 1:nrow(bottle)) {
#   # store values
#   nisk <- ncp_gop$niskin[i]
...
#   
#   # add parameters from API to TOI data frame
#   ncp_gop$datetime_utc_API[i] <- smry$date
#   ncp_gop$depth_API[i] <- smry$depth
#   ncp_gop$latitude_API[i] <- smry$latitude
#   ncp_gop$longitude_API[i] <- smry$longitude
# }
# 
# ncp_gop <- ncp_gop %>%
#   mutate(datetime_utc = case_when(toi_source =="toi_underway" ~ datetime_utc_matlab,
...
# 
# 
# # Add latitude and longitude from Underway API
# 
# ```{r}
# 
# # read in bottle summary for en617
# underway <- read_from_api(type = "summary", cruises = "EN617")
# 
