...
# if don't need to regenerate bottle file, start two chunks down with bottle <- read_csv("toi_bottle_before_qc.csv")
...
write.csv(bottle, "toi_bottle_before_qc.csv", row.names = FALSE)

```

```{r}

#bottle <- read_csv("toi_bottle_before_qc.csv")
...
# provide CTD metadata for AR28B cast 1 (there is no CTD btl file)
 
bottle <- bottle %>% mutate(cast = case_when(datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 1 ~ "1",
                                             datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 9 ~ "1",
                                            TRUE ~ cast)) %>%
                      mutate(latitude_API = case_when(datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 1 ~ 41.197,
                                                      datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 9 ~ 41.197,
                                                     TRUE ~ latitude_API)) %>%
                      mutate(longitude_API = case_when(datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 1 ~ -70.883,
                                                      datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 9 ~ -70.883,
                                                    TRUE  ~ longitude_API)) %>%
                      mutate(datetime_utc = case_when(datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 1 ~ datetime_utc_matlab,
                                                      datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 9 ~ datetime_utc_matlab,
                                                    TRUE  ~ datetime_utc)) %>%
                      mutate(depth = case_when(datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 1 ~ 15,
                                               datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 9 ~ 3,
                                                    TRUE  ~ depth)) %>%
                      mutate(depth_API = case_when(datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 1 ~ 15,
                                               datetime_utc_matlab == "2018-04-03 20:26:00" & niskin == 9 ~ 3,
                                                    TRUE  ~ depth_API)) 
...
ggplot(bottle, aes(x= datetime_utc_API, y = O2_Ar_ratio))+ geom_point()
ggplot(bottle, aes(x= datetime_utc_API, y = O2_Ar_delta))+ geom_point()
ggplot(bottle, aes(x= datetime_utc_API, y = cap_Delta_17O))+ geom_point()
ggplot(bottle, aes(x= datetime_utc_API, y = d17O))+ geom_point()
ggplot(bottle, aes(x= datetime_utc_API, y = d18O))+ geom_point()

# Plot measured parameters in bottle TOI dataset
ggplot(bottle, aes(x= cruise_day, y = O2_Ar_ratio, color = cruise))+ geom_point()
ggsave("toi-O2_Ar_ratio.png")
ggplot(bottle, aes(x= cruise_day, y = O2_Ar_delta, color = cruise))+ geom_point()
ggsave("toi-O2_Ar_delta.png")
ggplot(bottle, aes(x= cruise_day, y = cap_Delta_17O, color = cruise))+ geom_point()
ggsave("toi-cap_Delta_17O.png")
...
bottle_headers <- c("cruise", "datetime_utc","datetime_utc_input", "latitude_API", "longitude_API", "toi_source", "cast", "niskin", "depth", "depth_input", "O2_Ar_delta", "O2_Ar_ratio", "cap_Delta_17O", "d17O", "d18O")
...
ggplot(eims_all, aes(x= datetime_utc_matlab, y = salinity, color = cruise))+ geom_point()
ggplot(eims_all, aes(x= datetime_utc_matlab, y = temperature, color = cruise))+ geom_point()
ggplot(eims_all, aes(x= temperature, y = salinity, color = cruise))+ geom_point()
...
map_locs(df = eims_en608, xvar = "longitude_API", yvar = "latitude_API", region = "transect", color = "salinity")
map_locs(df = eims_ar28b, xvar = "longitude_API", yvar = "latitude_API", region = "transect", color = "salinity")
ggplot(eims_en608, aes(x= datetime_utc_matlab, y = salinity, color = temperature))+ geom_point()+ labs(title= "EN608")
ggplot(eims_ar28b, aes(x= datetime_utc_matlab, y = salinity, color = temperature))+ geom_point()+ labs(title= "AR28B")
ggplot(eims_en627, aes(x= datetime_utc_matlab, y = salinity, color = temperature))+ geom_point()+ labs(title= "EN627")
ggplot(eims_en649, aes(x= datetime_utc_matlab, y = salinity, color = temperature))+ geom_point()+ labs(title= "EN649")
...
ggplot(eims_ar28b, aes(x= salinity, y = biosat, color = datetime_utc_matlab))+ geom_point()+labs(title = "AR28B")
ggsave("ar28b-salinity-biosat.png")
...
ggplot(eims_at46, aes(x= salinity, y = biosat, color = datetime_utc_matlab))+ geom_point()+labs(title = "AT46")
...
#eims_all <- read_csv(paste0(here(), "/eims-toi-transect/eims-transect.csv"))

# separate data by year
eims_all$year <- format(eims_all$datetime_utc, "%Y")
...
for (i in 1:nrow(cruise_list)) {
  ...
  eims_year$cruise <- toupper(eims_year$cruise)
...
add_parent(edi_pkg = pkg_id, parent_name = "parent_project_NESI-II_RAPID.txt", xml.path = project_folder)
...
for (i in 1:nrow(this_pkg)){
  ...
  (this_cruise$discreterates_cols == "11"){
      ...
      ncp_gop_input <- subset(ncp_gop_input, select = -blank)
    }
...
for (i in 1:nrow(this_pkg)){
  ...
  # provide column names
  colnames(ncp_hifreq_input) <- c("datetime_utc_matlab", "O2_Ar_ratio", "temperature", "salinity", "latitude_matlab", "longitude_matlab", "cumulative_dist", "biosat", "ncp", "k")
  
    # replace date and time for samples exactly at midnight
    ncp_hifreq_input$date_utc_matlab <- as.POSIXct(ncp_hifreq_input$datetime_utc_matlab, format="%d-%b-%Y")
    ncp_hifreq_input$date_utc_matlab <- as_date(ncp_hifreq_input$date_utc_matlab)
    ncp_hifreq_input$date_utc_matlab <- as.character(ncp_hifreq_input$date_utc_matlab)
    
    ncp_hifreq_input$time_utc_matlab <- as.POSIXct(ncp_hifreq_input$datetime_utc_matlab, format="%d-%b-%Y %H:%M:%OS")
    ncp_hifreq_input$time_utc_matlab <- strftime(ncp_hifreq_input$time_utc_matlab, format="%H:%M:%OS")
    ncp_hifreq_input$time_utc_matlab <- replace(ncp_hifreq_input$time_utc_matlab, is.na(ncp_hifreq_input$time_utc_matlab), "00:00:01")
    
    ncp_hifreq_input$datetime_utc_midnights <- paste(ncp_hifreq_input$date_utc_matlab, ncp_hifreq_input$time_utc_matlab)
    
    # set date time format
    ncp_hifreq_input$datetime_utc_matlab <- as.POSIXct(ncp_hifreq_input$datetime_utc_midnights,format="%Y-%m-%d %H:%M:%OS")
   
    
    # remove unneeded columns
    #ncp_hifreq_input <- ncp_hifreq_input %>% select(-date_utc_matlab, -date_string, -time_utc_matlab, - datetime_utc_midnights)
...
sum(is.na(ncp_hifreq$datetime_utc_matlab))
...
sum(is.na(ncp_gop_input$datetime_utc_matlab)) 

# remove any rows with missing datetime
ncp_gop_input <- ncp_gop_input %>% filter(!is.na(datetime_utc_matlab))
...
# Set up to bind low-frequency data with TOI data to identify TOI_source and supply API-provided timestamp, latitude, and longitude
...
# deeper depth for package 32.1
toi_transect_surface <- filter(toi_transect, depth_input <= 7.1)


# remove rows not in in NCP-GOP data
# AR28B underway
if (this_year == 2018){
toi_transect_surface <- toi_transect_surface[-which(toi_transect_surface$datetime_utc_input == "2018-02-01 18:12:00"| 
                            toi_transect_surface$datetime_utc_input == "2018-02-03 03:50:00"),]
toi_transect_surface <- toi_transect_surface[-which(toi_transect_surface$cruise == "AR28B" & toi_transect_surface$toi_source == "toi_underway"),]
}

if (this_year == 2019){
toi_transect_surface <- toi_transect_surface[-which(toi_transect_surface$cruise == "AR34B" & toi_transect_surface$niskin == 15 ),]
 }

 if (this_year == 2021){
toi_transect_surface <- toi_transect_surface[-which(toi_transect_surface$datetime_utc_input == "2021-02-04 13:12:00"| 
                            toi_transect_surface$datetime_utc_input == "2021-02-04 18:53:00"| 
                            toi_transect_surface$datetime_utc_input == "2021-02-05 11:57:00"| 
                            toi_transect_surface$datetime_utc_input == "2021-02-05 23:30:00"| 
                            toi_transect_surface$datetime_utc_input == "2021-02-07 22:06:00"),]
 }

if (this_year == 2022){
toi_transect_surface <- toi_transect_surface[-which(toi_transect_surface$datetime_utc_input == "2022-08-03 01:34:00"),]
 }
...
write.csv(toi_transect_surface, "toi_transect_surface.csv")
...
#cruise_check_ncp <- ncp_gop_input %>% rename(datetime_utc_input = datetime_utc_matlab)
diffrows_ncp <- anti_join(ncp_gop_input, toi_transect_surface, by = "datetime_utc_input")
diffrows_toi <- anti_join(toi_transect_surface, ncp_gop_input,  by = "datetime_utc_input")


```

# complete bind of ncp_gop and TOI transect surface metadata
...
if (nrow(toi_transect_surface) == nrow(ncp_gop_input)){
  ...
  toi_transect_surface <- toi_transect_surface %>%
    rename(datetime_utc_input_toi = datetime_utc_input) %>%
    rename(depth_input_toi = depth_input) 
 # %>%    rename(iode_quality_flag_toi = iode_quality_flag)
  
   ncp_gop_input <- ncp_gop_input %>%
     rename(depth_input_ncp = depth) %>%
     rename(niskin_input_ncp = niskin)
  
  # ensure datetime_utc_matlab and datetime_utc_matlab_toi are in order
  ncp_gop_input <- ncp_gop_input[order(ncp_gop_input$datetime_utc_matlab),]
  toi_transect_surface <- toi_transect_surface[order(toi_transect_surface$datetime_utc_input_toi),]
  
  # bind
  ncp_gop_transect_wide <- cbind(ncp_gop_input, toi_transect_surface)
  
} else 
{print('ncp_gop_input rows not equal to toi_transect_surface rows')}
...
ncp_gop_transect_wide <- ncp_gop_transect_wide %>%
  mutate(timecheck = case_when(datetime_utc_matlab == datetime_utc_input_toi ~ 0,
                              datetime_utc_matlab != datetime_utc_input_toi ~ 1))
sum(ncp_gop_transect_wide$timecheck)  
bad_wide <- ncp_gop_transect_wide %>% filter(timecheck == 1)

ncp_gop_transect_wide <- ncp_gop_transect_wide %>%
    rename(latitude_input = latitude_matlab) %>%
    rename(longitude_input = longitude_matlab)

# reorder columns and drop toi data
gopheaders <- c("cruise", "datetime_utc","datetime_utc_input", "latitude_input", "longitude_input", "latitude_API", "longitude_API","toi_source", "gop", "ncp", "ncp_per_gop", "iode_quality_flag")
...
ggsave(paste0(project_folder, "/gop-point-", this_year, ".png"))
ggplot(ncp_gop_transect_wide, aes(x= cruise_day, y = ncp, color = cruise))+ geom_point()
ggsave(paste0(project_folder, "/ncp-point-", this_year, ".png"))
ggplot(ncp_gop_transect_wide, aes(x= cruise_day, y = ncp_per_gop, color = cruise))+ geom_point()
ggsave(paste0(project_folder, "/ncp-per-gop-point-", this_year, ".png"))

ggplot(ncp_gop_transect_wide, aes(x= cruise_day, y = gop, color = cruise))+ geom_line()
ggsave(paste0(project_folder, "/gop-line-", this_year, ".png"))
ggplot(ncp_gop_transect_wide, aes(x= cruise_day, y = ncp, color = cruise))+ geom_line()
ggsave(paste0(project_folder, "/ncp-line-", this_year, ".png"))
ggplot(ncp_gop_transect_wide, aes(x= cruise_day, y = ncp_per_gop, color = cruise))+ geom_line()
ggsave(paste0(project_folder, "/ncp-per-gop-line-", this_year, ".png"))
...
eims_transect <- eims_transect %>%
  rename(datetime_utc_eims = datetime_utc) %>%
  rename(latitude_eims = latitude) %>%
  rename(longitude_eims = longitude) %>%
...
eims_transect <- eims_transect[order(eims_transect$datetime_utc_eims),]
...
ncp_hifreq_wide <- ncp_hifreq_wide %>%
  mutate(timecheck = case_when(datetime_utc_matlab == datetime_utc_eims ~ 0,
                              datetime_utc_matlab != datetime_utc_eims ~ 1))
...
ncp_hifreq_wide <- ncp_hifreq_wide %>%
  ...
  rename(latitude = latitude_matlab) %>%
  rename(longitude = longitude_matlab) %>%
  rename(depth = depth_eims)

#target columns
ncp_hifreq_headers <- c("cruise", "datetime_utc", "latitude", "longitude", "cumulative_dist", "temperature", "salinity", "depth", "biosat", "O2_Ar_ratio", "ncp", "k")
...
ggplot(ncp_hifreq_wide, aes(x= cumulative_dist, y = biosat, color = cruiseid))+ geom_point()
ggsave(paste0(project_folder, "/biosat-", this_year, ".png"))
ggplot(ncp_hifreq_wide, aes(x= cumulative_dist, y = O2_Ar_ratio, color = cruiseid))+ geom_point()
ggsave(paste0(project_folder, "/O2_Ar_ratio-", this_year, ".png"))
ggplot(ncp_hifreq_wide, aes(x= cumulative_dist, y = ncp, color = cruiseid))+ geom_point()
ggsave(paste0(project_folder, "/ncp_hifreq-", this_year, ".png"))
ggplot(ncp_hifreq_wide, aes(x= cumulative_dist, y = k, color = cruiseid))+ geom_point()
ggsave(paste0(project_folder, "/k-", this_year, ".png"))

# would be helpful to color by season
# EN661 does not have cumulative_dist
...
map_locs(df = ncp_gop_edi, xvar = "longitude_input", yvar = "latitude_input",
         region = "transect", colorvar = "cruise")

# ncp high frequency
map_locs(df = ncp_hifreq_edi, xvar = "longitude", yvar = "latitude",
         region = "transect", colorvar = "cruise")
...
ncp_hifreq_headers <- c("cruise", "datetime_utc", "latitude", "longitude", "depth","temperature", "salinity", "ncp", "k")
...
# define input files
edi_filename <- paste0('ncp-gop-transect-', this_year)
meta <- paste0(project_folder, '/', edi_filename, '-info.xlsx')
...
sheet_to_tsv(meta, 'CategoricalVariables',
             glue::glue(project_folder, '/catvars_gop-transect-',this_year, '.txt'))

sheet_to_tsv(meta, 'ColumnHeadersGop',
             glue::glue(project_folder, '/attributes_gop-transect-',this_year, '.txt'))

sheet_to_tsv(meta, 'ColumnHeadersNcp',
...
gop_edi <- read_csv(paste0(here(project_folder, gop), ".csv")) #gop-transect
ncp_edi <- read_csv(paste0(here(project_folder, ncp), ".csv")) #ncp-transect
...
make_eml(path = project_folder,
         ...
         geographic.description = "NES-LTER Transect",
         geographic.coordinates = geographic_coordinates(lat, long),
         maintenance.description = "completed", #TODO: add column in cruise list to designate completed pkgs
         user.id = "NES",
         user.domain = "LTER",
         package.id = pkg_id)
...
# for prep-eims-toi-data: 
# 2018, 2019, 2021, 2022
add_parent(edi_pkg = pkg_id, parent_name = "parent_project_NESI-II.txt", xml.path = project_folder)
