...
Issue: The datetime_utc for bottle-sampled data represents the cast start time, NOT the time at which the bottle was fired. 
...
Other Issues: Correct depth field
```{r}
...
depth_diff <- abs(bottle$depth - bottle$depth_api)
depth_conflicts <- bottle[which(depth_diff > 1),]
...
for (i in 1:nrow(bottle)) {
  ...
  # store bottle time 
  bottle$latitude[i] <- underway$latitude[ind]
  bottle$longitude[i] <- underway$longitude[ind]
}
